...
func findLowestCost(tally map[int]int, sortedPositions []int) (int, int) {
	...
	for position := start; position <= end; position++ {
		cost := calcTotalCost(tally, position)
		if cost < bestCost {
...
func calcTotalCost(tally map[int]int, target int) int {
	totalCost := 0
	for position, count := range tally {
		distance := int(math.Abs(float64(target - position)))
		totalCost += calcCost(distance, count)
	}

	return totalCost
}

func calcCost(totalDistance int, count int) int {
	costSum := 0
	for dist := 1; dist <= totalDistance; dist++ {
		costSum += dist
	}
